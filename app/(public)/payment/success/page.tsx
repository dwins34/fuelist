...
function WhatsAppNotify({ order }: { order: StoredOrder }) {
  ...
  // Freeze the URL in a ref — must never change after mount so the effect
  // has no dependencies and only runs once.
  const urlRef = useRef(
  ...
  // Watch countdown separately so window.open is never inside a state updater
  useEffect(() => {
  ...
  return (
    <button
      onClick={handleClick}
      className="flex w-full items-center justify-between gap-3 rounded-xl bg-[#25D366] px-4 py-3 text-white shadow-md shadow-green-200 hover:bg-[#20b858] active:scale-95 transition-all"
    >
      ...
      <svg className="h-9 w-9 shrink-0 -rotate-90" viewBox="0 0 36 36">
        ...
        <circle cx="18" cy="18" r="15" fill="none" stroke="white" strokeWidth="3"
          strokeDasharray={`${(countdown / 5) * 94} 94`}
          strokeLinecap="round"
          style={{ transition: 'stroke-dasharray 1s linear' }}
        />
        <text x="18" y="22" textAnchor="middle" fill="white" fontSize="11" fontWeight="bold" className="rotate-90 origin-center" style={{ transform: 'rotate(90deg)', transformOrigin: '18px 18px' }}>
          {countdown}
        </text>
      </svg>
    </button>
